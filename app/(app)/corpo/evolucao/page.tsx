    ...
    return (
      <div className="min-h-screen bg-background pb-24">
        <div className="px-4 pt-12">
          ...
          <h1 className="text-2xl font-bold text-foreground mb-2">Análise de Evolução</h1>
        </div>

        <div className="px-4 mt-12 text-center">
          <div className="bg-white border border-border rounded-2xl p-8">
            <TrendingUp className="w-10 h-10 text-foreground-muted mx-auto mb-3" />
            <p className="text-foreground-secondary">Você precisa de pelo menos 2 medições para ver a evolução</p>
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      ...
      <div className="px-4 pt-12 pb-6">
        ...
        <h1 className="text-2xl font-bold text-foreground">Análise de Evolução</h1>
        <p className="text-foreground-secondary text-sm">{evolution.periodoTotal}</p>
      </div>
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-foreground-secondary">Período de acompanhamento</span>
            </div>
            <span className="text-foreground font-semibold">{evolution.diasAcompanhamento} dias</span>
          </div>

          <div className="text-center py-4 border-t border-border">
            <p className="text-foreground-muted text-xs mb-1">Número de medições</p>
            <p className="text-3xl font-bold text-foreground">{measurements.length}</p>
          </div>
        </motion.div>
      </div>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-foreground-secondary text-sm mb-1">{metric.label}</p>
          ...
          >
            ...
            <div className="space-y-4">
              ...
              {goals.peso_meta && (
                <div>
                  <div className="flex justify-between text-sm mb-2">
                    <span className="text-foreground-secondary">Peso</span>
                    <span className="text-foreground">
                      {evolution.evolucaoPeso?.valorAtual.toFixed(1)}kg
                      <span className="text-foreground-muted"> / {goals.peso_meta}kg</span>
                    </span>
                  </div>
              ...
              {goals.gordura_meta && (
                <div>
                  <div className="flex justify-between text-sm mb-2">
                    <span className="text-foreground-secondary">Gordura</span>
                    <span className="text-foreground">
                      {evolution.evolucaoGordura?.valorAtual.toFixed(1)}%
                      <span className="text-foreground-muted"> / {goals.gordura_meta}%</span>
                    </span>
                  </div>
                  <div className="h-2 bg-background rounded-full overflow-hidden">
                    <motion.div
              ...
              {goals.musculo_meta && (
                <div>
                  <div className="flex justify-between text-sm mb-2">
                    <span className="text-foreground-secondary">Músculo</span>
                    <span className="text-foreground">
                      {evolution.evolucaoMusculo?.valorAtual.toFixed(1)}kg
                      <span className="text-foreground-muted"> / {goals.musculo_meta}kg</span>
                    </span>
                  </div>
                  <div className="h-2 bg-background rounded-full overflow-hidden">
                    <motion.div
              ...
              {evolution.marcos.slice(0, 5).map((marco, index) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className="text-foreground font-medium">{marco.titulo}</p>
                    <p className="text-foreground-muted text-xs">{marco.descricao}</p>
