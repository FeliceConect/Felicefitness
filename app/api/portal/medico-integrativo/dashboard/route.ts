...
export async function GET() {
  try {
    ...
    if (!professional || professional.type !== 'medico_integrativo') {
      return NextResponse.json({ success: false, error: 'Acesso restrito a médico integrativo' }, { status: 403 })
    }
    ...
    console.error('Erro no dashboard médico integrativo:', error)
